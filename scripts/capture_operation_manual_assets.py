...
def main() -> None:
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            page.get_by_text("VCA Multi-Agent Synthetic Researcher").wait_for(timeout=60000)
            page.get_by_text("Model provider", exact=True).wait_for(timeout=60000)
            page.get_by_text("Edit research objective, hypotheses and decision rule", exact=True).wait_for(timeout=60000)
            page.wait_for_timeout(1200)
            ...
            if not args.skip_run:
                ...
                page.get_by_text("Customer Perspective Board", exact=True).scroll_into_view_if_needed()
                page.wait_for_timeout(700)
                screenshot(page, "11_synthetic_customer_lens.png")
                page.get_by_text("Consultant Quality Layer", exact=True).scroll_into_view_if_needed()
                page.wait_for_timeout(700)
                screenshot(page, "12_consultant_quality_layer.png")
                page.get_by_role("button", name="Download Consultant Delivery").scroll_into_view_if_needed()
                page.wait_for_timeout(700)
                screenshot(page, "13_delivery_pack.png")
